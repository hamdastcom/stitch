...
function VideoDetailDialog({
  ...
  const playUrl = getGenerationVideoUrl(selectedGeneration);
  const thumbUrl = getGenerationThumbnailUrl(selectedGeneration);
  ...
  const handleDelete = async (generationId) => {
    ...
    if (!ok) {
      toastUserError(data?.message, "خطا در حذف ویدیو");
      return;
    }
      ...
      >
        {selectedGeneration && (
          <div className="space-y-6">
            {/* Video Player */}
            <div className="relative rounded-xl overflow-hidden h-64  border border-border bg-secondary">
              {selectedGeneration.status === "completed" && playUrl ? (
                <video
                  controls
