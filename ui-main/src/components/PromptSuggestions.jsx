...
const quickSuggestions = [
  // { id: "caption", text: "یه کپشن باحال برای عکس اینستاگرامم بنویس" },
  { id: "games", text: "چندتا بازی برای دورهمی‌ها پیشنهاد بده" },
  // { id: "image", text: "یه عکس انیمه‌ای از رونالدو و امباپه بساز" },
  { id: "math", text: "اين معادله رو حل كن: x + 5 = 15" },
];
...
function PromptSuggestions({ onSelectPrompt }) {
  ...
  const handleCaptionImageUpload = async () => {
    // Use the Hamdast upload API to get a properly uploaded image
  ...
  const handleQuickSuggestionClick = (suggestion) => {
    if (suggestion.id === "image") {
      ...
      // Trigger the Hamdast image upload
